...
kp1, des1 = sift.detectAndCompute(gray1_clahe, None)
kp2, des2 = sift.detectAndCompute(gray2_clahe, None)
...
if len(good) > MIN_MATCH_COUNT:
    ...
    for y in range(0, h, block_size):
        for x in range(0, w, block_size):
            ...
            # diff_opening = cv2.morphologyEx(difference, cv2.MORPH_OPEN, kernel)
    ...
    _, thresh = cv2.threshold(difference, 40, 255, cv2.THRESH_BINARY)
    # show_1Img(thresh)
    ...
    cv2.drawContours(image1_with_diff, contours, -1, (0, 0, 255), 2)
